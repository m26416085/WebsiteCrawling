...
<body>
    ...
    ?>
    <form action="testgrafik.php" method="POST">
...
<?php
    ...
    foreach ($data["data"] as $profil) {
        ...
        $item_price=(int)$data["data"][$i]["product"]["price"];
        $shop_name=(string)$data["data"][$i]["shop"]["name"];
        array_push($dataPoints, array("y" =>$item_price , "label" =>$shop_name ));
        $i = $i + 1;
    }
    echo "<pre>";
...
<!-- Grafik -->
<button onclick="creategrafik()">Create Grafik</button>
